...
public class JZExoPlayer extends JZMediaInterface implements Player.EventListener, VideoListener {
    ...
    public JZExoPlayer(Context context) {
    ...
    public void onVideoSizeChanged(int width, int height, int unappliedRotationDegrees, float pixelWidthHeightRatio) {
        ...
        //note undo this if to not support the long display devices i.e. with bottom soft navigation button
        Log.i(TAG, "onVideoSizeChanged: width height "+width+" "+height);
        if ((float)height/width >= 1.7) {

            if (DisplayHelper.hasNavBar(context)) {

                JZMediaManager.instance().currentVideoWidth = getwidth();
                JZMediaManager.instance().currentVideoHeight = getheight() - DisplayHelper.dp2px(context,54);

            } else {

                JZMediaManager.instance().currentVideoWidth = getwidth();
                JZMediaManager.instance().currentVideoHeight = getheight();

            }
            Log.i(TAG, "onVideoSizeChanged: ar");
...
//        });
                ...
                if (JzvdMgr.getCurrentJzvd() != null) {
                    switch (playbackState) {
                        ...
                        case Player.STATE_BUFFERING: {
                            JzvdMgr.getCurrentJzvd().onStatePreparing();
                            mainHandler.post(callback);
                            Log.i(TAG, "run: Preaparing");
                        }
                        break;
    ...
    private int getheight() {
        ...
        //int width = size.x;
        int height = size.y;
        Log.d(TAG, "getheight() returned: " + height);
    ...
    private int getwidth() {
        ...
        display.getRealSize(size);
        int width = size.x;
        Log.d(TAG, "getwidth() returned: " + width);
        //int height = size.y;
        /*DisplayMetrics displayMetrics = context.getResources().getDisplayMetrics();
